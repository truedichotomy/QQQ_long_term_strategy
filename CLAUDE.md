...
- **`analysis/` root — production (the adopted overlay).** `signal.jl` **auto-pulls the latest data** (via `src/fetch.jl` → Yahoo, `--no-fetch` to skip), merges it into `data/`, prints today's buy-hold/sell-wait call, and refreshes `web/data.js`. The adopted strategy is the **either-on blend** (`blend_either_on`): long if EITHER a 50/200 SMA fast-reentry **trend** filter OR a CCI(40)/TMA(200) **momentum** filter (`cci_band_tma_switch`) is long — they fail in opposite regimes, so the blend covers both. `blend_avg` is the smoother ½-size alternative. Also here: `fetch_data.jl` (explicit fetch/inspect; same `src/fetch.jl`), `build_blend_pages.jl`, `blend_finalists_compare.jl`, `blend_walkforward.jl`, `plot_blend_finalists.jl`. The auto-pull writes a fixed git-ignored "live overlay" file (`data/QQQ (99999999999999999 _ …).csv`) that always wins `load_ticker`'s date-dedupe. Execution guide: `analysis/STRATEGY.md`; study writeup: `analysis/README.md`.
