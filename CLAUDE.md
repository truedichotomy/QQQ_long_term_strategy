...
julia --startup-file=no analysis/fetch_data.jl              # refresh data/ from a free source (last 10 trading days)
...
- `web/` — a browser version of the signal (plain HTML/CSS/JS, no server). The QQQ history is **bundled** in `web/data.js`, so opening `web/index.html` shows the latest signal immediately; dropping a newer file merges it into a deduped `localStorage` database (uploads accumulate, newest wins on overlapping dates). `web/strategy.js` is a faithful port of `src/{indicators,strategies}.jl`, verified against `signal.jl` via `node web/verify_node.js`. After updating `data/`, regenerate the bundle with `node web/build_data.js`. Keep the port in sync if the adopted strategy or data format changes.
...
- **`analysis/` root — production (the adopted overlay).** `signal.jl` prints today's buy-hold/sell-wait call. The adopted strategy is the **either-on blend** (`blend_either_on`): long if EITHER a 50/200 SMA fast-reentry **trend** filter OR a CCI(40)/TMA(200) **momentum** filter (`cci_band_tma_switch`) is long — they fail in opposite regimes, so the blend covers both. `blend_avg` is the smoother ½-size alternative. Also here: `fetch_data.jl` (pull recent OHLCV from Yahoo via stdlib `Downloads` into `data/` — no API key/site; writes a `load_ticker`-format CSV that merges by date), `build_blend_pages.jl`, `blend_finalists_compare.jl`, `blend_walkforward.jl`, `plot_blend_finalists.jl`. Execution guide: `analysis/STRATEGY.md`; study writeup: `analysis/README.md`.
