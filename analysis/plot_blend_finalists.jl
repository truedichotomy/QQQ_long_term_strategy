# Chart: the two final blend candidates vs buy & hold (1999–2026).
# Two stacked panels — growth of $1 (log) and drawdown. Pure base Julia SVG.
#   avg (½-size)  : exposure = (CCI40 TMA-trigger + SMA fast-reentry)/2
#   either-on     : long if EITHER underlying rule is long
#
#   julia analysis/plot_blend_finalists.jl   # -> results/blend_finalists.svg (+ .png if rsvg present)
...
const SVG = joinpath(OUTDIR, "blend_finalists.svg")
...
curves = [
    ...
    ("blend: avg (½)",   "#1f6feb", bt((CCI .+ FR) ./ 2)),
    ("blend: either-on", "#d1242f", bt(sig_or(CCI, FR))),
]
...
<text x="$L" y="20" font-size="17" font-weight="bold">$(yr[1])–$(yr[end]) QQQ blends: avg (½-size) vs either-on vs buy &amp; hold</text>
""")
...
let yrs = (Dates.value(d.date[end] - d.date[1])) / 365.25,
    ...
    print(io, "<text x=\"$lx\" y=\"$(ly+150)\" font-size=\"11\" fill=\"#1f6feb\">avg: shallower in every crash</text>")
    print(io, "<text x=\"$lx\" y=\"$(ly+166)\" font-size=\"11\" fill=\"#d1242f\">either: more \$, ~3 trades/yr</text>")
end
print(io, "</svg>")
write(SVG, take!(io))
println("Wrote ", SVG)
