# The adopted QQQ blend overlay on REAL daily DJIA, 1970-2026 — a genuine
# out-of-sample test on a different (and choppier) index than QQQ. Same strategy
# code (blend_either_on / blend_avg from src/strategies.jl), same engine, same
# parameters. DJIA is a price index (no dividends); cash earns rf while flat.
using Dates, Printf, Statistics
include(joinpath(dirname(@__DIR__), "QQQBacktest.jl")); using .QQQBacktest

d = load_ticker("DJI"; dir=joinpath(dirname(dirname(@__DIR__)), "data"))
dates, close = d.date, d.close
@printf("DJI: %s .. %s  (%d days)  %.0f -> %.0f\n", dates[1], dates[end], length(d), close[1], close[end])
...
# buy & hold worst drawdown and the overlays' drawdown in the same window
...
# regime sub-period table: total return over span / deepest drawdown within
...
regimes = [("1973-74 bear",      Date(1973,1,1),  Date(1974,12,31)),
           ("1980s bull",        Date(1982,8,1),  Date(1987,8,31)),
           ("1987 crash",        Date(1987,8,1),  Date(1987,12,31)),
           ("1990s bull",        Date(1990,10,1), Date(2000,1,14)),
           ("dot-com crash",     Date(2000,1,14), Date(2002,10,9)),
           ("GFC",               Date(2007,10,9), Date(2009,3,9)),
           ("2009-2020 bull",    Date(2009,3,9),  Date(2020,2,12)),
           ("COVID crash",       Date(2020,2,12), Date(2020,3,23)),
           ("2022 bear",         Date(2022,1,3),  Date(2022,10,12)),
           ("FULL 1970-2026",    dates[1],        dates[end])]
...
# rolling 5-year win-rate vs buy & hold (return-beat / shallower-drawdown)
